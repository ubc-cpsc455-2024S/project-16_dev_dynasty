...
router.post("/:houseId", upload.array("images", 10), async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Defect created successfully", defect: newDefect });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const defectIndex = house.defects.findIndex(
        (def) => def._id.toString() === req.params.defectId,
      );
      if (defectIndex === -1)
      ...
      if (imagesToDelete.length > 0) {
        ...
        defect.images = defect.images.filter(
      ...
      if (req.files.length > 0) {
        defect.images = defect.images.concat(
...
router.delete("/:houseId/:defectId", async (req, res) => {
  try {
    ...
    const defectIndex = house.defects.findIndex(
      (defect) => defect._id.toString() === defectIdStr,
    );
