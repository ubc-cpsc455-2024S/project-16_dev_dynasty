...
import { fetchDocumentsByHouseId, addDocumentAsync, updateDocumentAsync, deleteDocumentAsync } from './thunksDocuments';
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateDocumentAsync.fulfilled, (state, action) => {
        state.loading = false;
        const index = state.list.findIndex(document => document._id === action.payload._id);
        if (index !== -1) {
      ...
      .addCase(deleteDocumentAsync.fulfilled, (state, action) => {
        state.loading = false;
        state.list = state.list.filter(document => document._id !== action.payload._id);
      })
